...
class GoogleDocsClient:
    ...
    def export_tab_html(self, doc_id: str, tab_id: str) -> str:
        try:
            ...
            response = requests.get(url, params={"format": "html", "tab": tab_id}, timeout=15)
            response.raise_for_status()
            
        except requests.RequestException as exc:
            raise GoogleDocsFetchError(f"Failed to fetch tab '{tab_id}' from doc '{doc_id}': {exc}") from exc
