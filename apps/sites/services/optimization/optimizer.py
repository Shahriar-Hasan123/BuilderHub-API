...
def compress_image(
    ...
    if result.file_size >= original_size and not dimension_capped:
    ...
    if not result.meets_target():
        raise RuntimeError(
            f"Target guarantee violated: output {result.file_size} bytes "
            f"> target {target_bytes} bytes for '{original_name}'."
        )

    # 3. Enforce smaller-than-original rule if original did not meet target
    if not result.is_smaller_than(original_size) and not dimension_capped:
        raise RuntimeError(
            f"Compression produced a larger file ({result.file_size} bytes) "
            f"than source ({original_size} bytes) for '{original_name}'."
        )

    if result.width <= 0 or result.height <= 0:
        raise RuntimeError(
            f"Invalid output dimensions {result.width}x{result.height} "
            f"for '{original_name}'."
        )
