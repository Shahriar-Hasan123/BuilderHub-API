...
Rollback restores editable source from a selected historical version and republishes the restored state as a new version. The original historical version is preserved.

## Main Components

- `PublishService`: validates, builds, publishes, compares, restores, and rolls back versions.
...
8. Write `header.json`, `footer.json`, and page JSON files to the site's asset directory.
...
11. Create or reuse a new publish version for the restored state.
...
- the restored state is published again as a new version
```

The original Version 1 remains unchanged. The new publish represents the rollback result in the version history.
...
- A new version created from every successful rollback result
